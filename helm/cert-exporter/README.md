# Cert Exporter Helm Chart

这是一个用于部署SSL证书过期监控 Prometheus Exporter 的 Helm Chart。
...
```bash
# 添加 Helm 仓库（如果有的话）
helm repo add cert-exporter https://your-helm-repo.com
...
```bash
# 使用自定义 values 文件
helm install cert-exporter ./cert-exporter -f ./cert-exporter/values-prod.yaml
...
```bash
helm install cert-exporter-dev ./cert-exporter -f ./cert-exporter/values-dev.yaml
```
...
|------|------|--------|
| `replicaCount` | 副本数量 | `1` |
| `image.repository` | 镜像仓库 | `ghcr.io/kevin197011/cert-exporter` |
...
helm install cert-exporter ./cert-exporter \
  ...
  --set ingress.hosts[0].host=cert-exporter.yourdomain.com \
...
```bash
kubectl get pods -l app.kubernetes.io/name=cert-exporter
```

### 查看日志

```bash
kubectl logs -l app.kubernetes.io/name=cert-exporter
```

### 查看服务

```bash
kubectl get svc -l app.kubernetes.io/name=cert-exporter
```

### 端口转发测试

```bash
kubectl port-forward svc/cert-exporter-cert-exporter 8080:8080
  ...
  - job_name: 'cert-exporter'
    ...
    relabel_configs:
      - source_labels: [__meta_kubernetes_service_name]
        action: keep
        regex: cert-exporter-cert-exporter
```

### Grafana 仪表板

可以创建 Grafana 仪表板来可视化SSL证书过期信息：
